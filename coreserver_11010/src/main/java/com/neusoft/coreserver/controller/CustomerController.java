...
import com.neusoft.coreserver.entity.*;
import com.neusoft.coreserver.feignclient.GatewayFeignClient;
...
public class CustomerController {
	...
	public void setNursingLevelMapper(NursingLevelMapper nursingLevelMapper) {this.nursingLevelMapper = nursingLevelMapper;}
	...
	public ResponseBean<Customer> load(HttpServletRequest httpServletRequest) {
		...
		try {
			if (httpServletRequest.getHeader("token") != null) {
				...
				Map<String, Object> map = new HashMap<>();
				map.put("token", token);
				String customerJson = gatewayFeignClient.parseToken(map);
				// json转化为对象
				ObjectMapper om = new ObjectMapper();
	...
	public ResponseBean<String> logout(HttpServletRequest httpServletRequest) {
		...
		try {
			if (httpServletRequest.getHeader("token") != null) {
				...
				String json = gatewayFeignClient.parseToken(requestMap);
				...
				if (map.containsKey("userId")) {
					...
					requestMap.put("key", "user-"+user.getUserId().toString());
					gatewayFeignClient.delete(requestMap);
				}
					...
					requestMap.put("key", "customer-"+customer.getCustomerId().toString());
					gatewayFeignClient.delete(requestMap);
				}
